...
matrix[p, k] Cbmf; // C parameter in BMF as described in Hoff (2009).
matrix[p,p] Abmf; // A parameter in BMF
matrix[k,k] Bbmf; // B parameter in BMF
}
...
model{
  target += calc_logJm(p, k, phi, Dk, K) + trace(Cbmf'*Q + Bbmf*Q'*Abmf*Q);
}
